...
function App() {
  ...
  //this is looks like a bug with routing. Without location decloration this is doesn't work
  //I know, that I never use useLocation hook!!!
  ...
  const onCompletedTask = (listId, taskId, completed) => {
    ...
    setLists(newList)
    axios.patch('http://localhost:3000/tasks/' + taskId, { completed })
      .catch(() => { alert('Error with task edition') })
  }
  ...
  const onEditTask = (listId, taskObj) => {
    ...
    setLists(newList)
    axios.patch('http://localhost:3000/tasks/' + taskObj.id, { text: newTaskText })
      .catch(() => { alert('Error with task edition') })
  }
  ...
  const onRemoveTask = (listId, taskId) => {
    if (window.confirm('Are you want to delete the task?')) {
      ...
      axios.delete('http://localhost:3000/tasks/' + taskId)
        .catch(() => { alert('Error with delete list title') })
    }
  }
